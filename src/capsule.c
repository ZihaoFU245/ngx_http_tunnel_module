...
static void capsule_chain_copy(ngx_chain_t **chain, u_char **pos, ngx_buf_t *dst,
							   size_t n);
...
tunnel_capsule_decode_datagram(ngx_chain_t **src, ngx_buf_t *dst)
{
	...
	if (capsule_chain_read_varint(&cl, &p, &capsule_len,
								  &len_len) != NGX_OK) {
		return NGX_AGAIN;
	}
	...
	if (capsule_chain_read_varint(&cl, &p, &context_id,
								  &context_len) != NGX_OK) {
		return NGX_DECLINED;
	}
...
capsule_is_protocol_header(ngx_table_elt_t *header)
{
	...
	key = (header->lowcase_key != NULL) ? header->lowcase_key
										: header->key.data;
